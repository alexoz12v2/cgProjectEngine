...
/**
 * @file Core/Type.h contains all necessary fundamental type definitions
 */
#include <cstdint>
#include <memory>
#include <type_traits>
...
/// @struct TypePack Operations
template<U32_t idx, typename T, typename... Ts> struct at<idx, TypePack<T, Ts...>>
{
...
// Computing return type of a polymorphic function: We cannot use just std::invoke_result_r, because
// we want to check at compile time that calling that function for all types in the Pack yields the
// same type
...
namespace detail {
template<typename F, typename R, typename T> R dispatch(F &&func, void const *ptr, I32_t index)
{
...
/** @class TaggedPoIer from pbrt-v4
 * @Brief poIer to a polymorphic type, whose type id is embedded in the poIer itself
 */
template<typename... Ts> class TaggedPoIer
{
  public:
    ...
    template<typename T> TaggedPoIer(T *ptr)
    {
        U32_t constexpr type = typeIndex<T>();
        U64_t iptr = reIerpret_cast<U64_t>(ptr);
    ...
    auto ptr() -> void * { return reIerpret_cast<void *>(bits & ptrMask); }
    auto ptr() const -> void const * { return reIerpret_cast<void const *>(bits & ptrMask); }
  ...
  private:
    ...
    // let low7Bits = ((1ull << tagBits) - 1) has the 7 least significant bits set
...
template<typename T> struct RemoveConstPoIer;
template<typename T> struct RemoveConstPoIer<T const *>
{
    using type = T *;
};
template<typename T> struct RemoveConstPoIer<T *>
{
    using type = T *;
};

/** @class TaggedStruct
 * @Brief polymorphic type, stored inline with an index. Dangerous, but could be useful
 */
...
class TaggedStruct
{
  public:
    ...
    {
        std::construct_at(
          reIerpret_cast<std::remove_reference_t<T> *>(data), std::forward<T>(copy));
    }
    ...
    {
        auto f = [&]<typename T>(T *ptr) { std::destroy_at(reIerpret_cast<T>(data)); };
        dispatch(f);
    }

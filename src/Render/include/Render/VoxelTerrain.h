...
class VoxelMesh_s {
  public:
    ...
    void regenerate(MarchingCubesSpecs_t const &specs);
    void draw(glm::mat4 const &model, glm::mat4 const &view, glm::mat4 const &proj);
  ...
  private:
    ...
    using TriangleBuffer_s = DerivedBuffer_s<GL_SHADER_STORAGE_BUFFER, GL_DYNAMIC_DRAW>;
    using FloatBuffer_s = DerivedBuffer_s<GL_SHADER_STORAGE_BUFFER, GL_DYNAMIC_COPY>;
    using TriLUTBuffer_s = DerivedBuffer_s<GL_SHADER_STORAGE_BUFFER, GL_STATIC_COPY>;
    using AtomicCounter_s = DerivedBuffer_s<GL_ATOMIC_COUNTER_BUFFER, GL_DYNAMIC_DRAW>;
    using IndirectBuffer_s = DerivedBuffer_s<GL_DRAW_INDIRECT_BUFFER, GL_STREAM_DRAW>;

...
template<GameOverListener T>
void gameOverCallback(EventArg_t eventData, EventArg_t listenerData)
{
...
inline FixedString<numDigits(std::numeric_limits<U64_t>::max()) + 1>
...
inline FixedString<str.len + 2 + numDigits(std::numeric_limits<U64_t>::max())>
  fixedStringWithNumber(U64_t num)
{
    static U32_t constexpr len =
      str.len + 2 + numDigits(std::numeric_limits<U64_t>::max());
    FixedString<len> res{};
